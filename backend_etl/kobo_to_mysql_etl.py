...
# ==========================================
# Fallback: If not found, look up one level to the project root directory
if not os.getenv("KOBO_TOKEN"):
    root_env_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".env"))
    load_dotenv(dotenv_path=root_env_path)

# Support both variations of your environment keys seamlessly
KOBO_TOKEN = os.getenv("KOBO_TOKEN")
ASSET_UID = os.getenv("KOBO_ASSET_UID") or os.getenv("ASSET_UID")

# Also align the data stream URL to match the exact non-trailing slash API v2 router rule
KOBO_URL = f"https://kf.kobotoolbox.org/api/v2/assets/{ASSET_UID}/data?format=json&limit=1000"

# KOBO_TOKEN = os.getenv("KOBO_TOKEN")
# ASSET_UID = os.getenv("ASSET_UID")
# KOBO_URL = f"https://kf.kobotoolbox.org/api/v2/assets/{ASSET_UID}/data.json"
...
DB_NAME = os.getenv("DB_NAME", "me_monitoring_db")

# Validate critical environment variables
if not KOBO_TOKEN or not ASSET_UID or not DB_PASSWORD:
    raise ValueError("Missing required environment variables: KOBO_TOKEN, ASSET_UID, or DB_PASSWORD")
